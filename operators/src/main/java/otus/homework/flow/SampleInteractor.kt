...
class SampleInteractor(
    ...
    fun task2(): Flow<String> {
        return sampleRepository.produceNumbers()
            .transform {
                when {
                    it % 15 == 0 -> { emit("$it") ; emit("FizzBuzz") }
                    it % 5 == 0 -> { emit("$it") ; emit("Buzz") }
                    it % 3 == 0 -> { emit("$it") ; emit("Fizz") }
                    else -> emit("$it")
                }
            }
    }
    ...
    fun task3(): Flow<Pair<String, String>> {
        val flow = sampleRepository.produceColors().zip(sampleRepository.produceForms()){ f1, f2 ->
            Pair(f1, f2)
        }
        return flow
    }
    ...
    fun task4(): Flow<Int> {
        return sampleRepository.produceNumbers()
            .catch { cause: Throwable -> if(cause is IllegalArgumentException) emit(-1) else throw cause }
