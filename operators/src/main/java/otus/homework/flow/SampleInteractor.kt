...
class SampleInteractor(
    ...
    fun task2(): Flow<String> {
        return sampleRepository.produceNumbers()
            .transform { result ->
                ...
                when {
                    result % 15 == 0 -> emit("FizzBuzz")
                    result % 3 == 0 -> emit("Fizz")
                    result % 5 == 0 -> emit("Buzz")
                }
            }
    }
    ...
    fun task3(): Flow<Pair<String, String>> {
        val colorsRepo = sampleRepository.produceColors()
        val formsRepo = sampleRepository.produceForms()
        return colorsRepo.zip(formsRepo) { color, form ->
            Pair(color, form)
        }
    }
