...
class SampleInteractor(
    ...
    fun task3(): Flow<Pair<String, String>> {
        val colorFlow = sampleRepository.produceColors()

        val formsFlow = sampleRepository.produceForms()
        val result = colorFlow.zip(formsFlow) { color, forms -> Pair(color, forms) }
    ...
    suspend fun task4(): Flow<Int> {
        val result =
            sampleRepository.produceNumbers().onCompletion { sampleRepository.completed() }.catch {
