...
class SampleInteractor(
     ...
     */
    fun task2(): Flow<String> = sampleRepository
        .produceNumbers().transform {
            emit("$it")
            if (it % 15 == 0) emit("FizzBuzz")
            else if (it % 5 == 0) emit("Buzz")
    ...
    fun task3(): Flow<Pair<String, String>> =
        sampleRepository.produceColors().zip(sampleRepository.produceForms()){ first, second ->
            Pair(first, second)
    }
     ...
     */
    fun task4(): Flow<Int> = sampleRepository
        .produceNumbers()
        .catch {
