...
class SampleInteractor(
     ...
     */
    fun task2() = sampleRepository.produceNumbers()
        .transform {
            emit("$it")
            when {
     ...
     */
    fun task3() = with(sampleRepository) {
        produceColors().zip(produceForms()) { color, form -> color to form }
    }
     ...
     */
    fun task4() = with(sampleRepository) {
        produceNumbers()
            .catch {
